...
abstract class ControlItem {
	...
	public function getHeaderName() {
		if (method_exists($this, "getHeaderForceName")) return $this->getHeaderForceName();
		$h = $this->getHeader();
	...
	public function getIcon() {
		if (method_exists($this, "getHeaderIcon")) return $this->getHeaderIcon();
		$h = $this->getHeader();
	...
	function getHtml($action, $id, $mode) {
		...
		if ($content instanceof Redirect) return $content;
	...
	static function listItems($max = 6) {
		...
		foreach (Platform::getSolutions("ControlItems") as $solution) {
			foreach ($solution->getFile()->listSubs() as $item) {
				...
				if ($obj->canView()) array_push($arr, $obj);
			}
		}
	...
	static function get($name) {
		foreach (self::listItems(99999) as $item) {
			if ($item->getId() == $name) return $item;
		}
		return null;
	}
}