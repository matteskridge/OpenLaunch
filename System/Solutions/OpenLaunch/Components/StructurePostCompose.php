...
<form action="admin/index/structure/posts/<?php echo $id ?>/?sid=<?php echo session_id() ?>" method="post" class="blog-post-form">
	<div class='blog-compose-settings'>
		<div class="blog-compose-settings-inner">
			Post Title:<input type='text' name='blogpost-name' value='<?php echo $name ?>' /><select name="blogpost-category">
				...
				<optgroup label="<?php echo $page->get("name") ?>">
					...
					<?php foreach (BlogCategory::findAll("BlogCategory", array("page" => $page)) as $c) { ?>
					<?php $cat2 = $page->getId().",".$c->getId() ?>
					<option value="<?php echo $cat2 ?>"<?php if ($cat == $cat2) { ?> selected="yes"<?php } ?>><?php echo $c->get("name") ?></option>
					<?php } ?>
				</optgroup>
				<?php } ?>
			</select>
		...
		<?php echo Component::get("OpenLaunch.RichTextEditor", array("id" => "blogpost-text", "value" => Parser::parse($content))) ?>
	</div>
</form>