<div class="admin-entry admin-entry-update">
	<div class="admin-entry-inner">
		<h2>Update OpenLaunch</h2>

		<p>This tool allows you to upgrade to the latest version of OpenLaunch.
			Simply click the below link to initiate an update. It may take several
			moments for the download to complete. Thank you!</p>

		<p><a href="admin/index/settings/update/?sid=<?php echo session_id() ?>">Update OpenLaunch Now</a></p>

	</div>
</div>